...
import { fetchWorkspaces, selectAllWorkspaces } from "@/features/workspace/workspaceSlice";
  ...
  return (
    <div>
      {workspaces.map((workspace) => (
        ...
        <button className="mr-2 text-neutral-400 hover:text-white" onClick={() => (
          setExpandedWorkspaceId(expandedWorkspaceId === workspace.id ? null : workspace.id)  
        )}>
          {expandedWorkspaceId === workspace.id ? <ChevronDown size={16} /> : <ChevronRight size={16}/>}
        </button>
