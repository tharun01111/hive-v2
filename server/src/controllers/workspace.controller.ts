...
export const createWorkspaceController = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.log(`[CREATE_WORKSPACE_SUCCESS] userId: ${userId} workspaceId: ${result.id}`);
...
export const deleteWorkspaceController = async (req: AuthRequest, res: Response, next: NextFunction) => { 
  ...
  try {
    ...
    console.log(`[DELETE_WORKSPACE_FAILED] userId: ${userId} workspaceId: ${workspaceId}`);
    console.error("Error in createWorkspace controller: ", error);
    next(error);
  }
...
export const getWorkspaceController = async (req: AuthRequest, res: Response, next: NextFunction) => {
  const userId = req.user?.id;
  ...
  console.log(`[GET_WORKSPACE_BY_ID_ATTEMPT] userId: ${userId} workspaceId: ${requestedWorkspaceId}`);

  try {
    const result = await workspaceService.getWorkspaceById({ userId, workspaceId: requestedWorkspaceId });

    console.log(`[GET_WORKSPACE_BY_ID_SUCCESS] userId: ${userId} workspaceId: ${requestedWorkspaceId}`);
    return res.status(200).json({ success: true, workspace: result });
  } catch (error) {
    console.error("Error in getUserWorkspaceControllerById: ", error);
    console.log(`[GET_WORKSPACE_BY_ID_FAILED] userId: ${userId} workspaceId: ${requestedWorkspaceId}`);
    next(error);
  }
}
