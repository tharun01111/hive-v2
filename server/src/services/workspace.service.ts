...
export const createWorkspace = async ({
  ...
  const result = await workspaceRepository.createWorkspace(
    userId,
    name,
    description,
  );
...
export const requireWorkspaceAccess = async (
  ...
  const workspace = await workspaceRepository.findAccessibleWorkspace(
    userId,
    workspaceId,
  );

  if (!workspace) throw new AppError("Not authorised for this workspace", 403);
...
export const getWorkspaceById = async ({
  ...
  const result = await workspaceRepository.findWorkspaceById(
    userId,
    workspaceId,
  );

  if (!result) throw new AppError("Workspace not found", 404);
